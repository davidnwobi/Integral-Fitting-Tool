...
def integrand(x, A, B, C, D):
    ...
    return (si1*be1 + si2*be2 + si2*be1)**2 * np.sin(x)
...
tuned_quad_dict, tuned_quad_rel_err, model = fit_model("core_shell_bicelle_high_res", integrand, a, b, params_names, reg_params2, rtol, update=True, degree=3, full_output=True, n_jobs=-2, plot_param_space=True)
